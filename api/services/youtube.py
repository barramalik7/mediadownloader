import sys
import os
from ..utils.helpers import get_downloads_dir, find_cookie, stream_download_command
import json
from typing import Generator

def download_youtube(url: str, quality: str = "1", format_choice: str = "mp4", cookies_dir: str = None, output_dir: str = None):
    """Download media from YouTube using yt-dlp."""
    try:
        if "youtube.com" not in url and "youtu.be" not in url:
            error_data = {"status": "error", "message": "Invalid YouTube URL"}
            yield f"data: {json.dumps(error_data)}\n\n"
            return

        quality_formats = {
            "1": "bestvideo[ext=mp4]+bestaudio[ext=m4a]/bestvideo+bestaudio[ext=m4a]/bestvideo+bestaudio/best",
            "2": "bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=1080]+bestaudio/best[height<=1080]",
            "3": "bestvideo[height<=720][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=720]+bestaudio/best[height<=720]",
            "4": "bestvideo[height<=480][ext=mp4]+bestaudio[ext=m4a]/bestvideo[height<=480]+bestaudio/best[height<=480]"
        }

        is_audio_only = format_choice.lower() == "mp3"
        format_string = "bestaudio[ext=m4a]/bestaudio/best" if is_audio_only else quality_formats.get(quality, quality_formats["1"])

        if output_dir is None:
            output_dir = get_downloads_dir("youtube")
        else:
            os.makedirs(output_dir, exist_ok=True)

        output_template = os.path.join(output_dir, "%(title)s.%(ext)s")

        cmd = [sys.executable, "-m", "yt_dlp", url, "-f", format_string, "-o", output_template, "--no-playlist", "--impersonate", "chrome-131:macos-14"]

        if is_audio_only:
            cmd.extend(["--extract-audio", "--audio-format", "mp3", "--audio-quality", "0"])
        else:
            cmd.extend(["--merge-output-format", "mp4"])

        cookie_path = find_cookie(cookies_dir, ["youtube.com_cookies.txt"])
        if cookie_path:
            cmd.extend(["--cookies", cookie_path])

        return stream_download_command(cmd)

    except Exception as e:
        error_data = {"status": "error", "message": f"Internal Server Error: {str(e)}"}
        yield f"data: {json.dumps(error_data)}\n\n"
